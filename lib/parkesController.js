...
const { createProxyHooks, hook, hookHasThrown } = require('./hook');
...
class ParkesController {
	...
	async show(ctx, next) {
		...
		_hook = await hook(this, 'beforeShow', ctx); // bind hook
		if (hookHasThrown(_hook)) throw _hook;

		const model = await this.rest.show(ctx);
		this.authorize(ctx, { model, action: 'show', scopes: true });

		_hook = await hook(this, 'afterShow', ctx, model); // bind hook
		if (hookHasThrown(_hook)) throw _hook;
	...
	async index(ctx, next) {
		...
		_hook = await hook(this, 'beforeIndex', ctx); // bind hook
		if (hookHasThrown(_hook)) throw _hook;
		...
		_hook = await hook(this, 'afterIndex', ctx, models.collection); // bind hook
		if (hookHasThrown(_hook)) throw _hook;
	...
	async create(ctx, next) {
		...
		let _hook = await hook(this, 'afterCreate', ctx, model); // bind hook
		if (hookHasThrown(_hook)) throw _hook;
	...
	async update(ctx, next) {
		...
		let _hook = await hook(this, 'afterUpdate', ctx, model); // bind hook
		if (hookHasThrown(_hook)) throw _hook;
	...
	async destroy(ctx, next) {
		...
		_hook = await hook(this, 'beforeDestroy', ctx, model); // bind hook
		if (hookHasThrown(_hook)) throw _hook;

		await this.rest.destroy(ctx, model);

		_hook = await hook(this, 'afterDestroy', ctx, model); // bind hook
		if (hookHasThrown(_hook)) throw _hook;
