...
class ParkesController {
	...
	async authorize(ctx, options) {
		...
		if (this.options.enableAsyncAuth === true) {
			// asyncronous auth
			await this.options.authorize(ctx, options);
		} else {
			// asyncronous auth
	...
	async index(ctx, next) {
		...
		await this.authorize(ctx, { action, model: this.rest.modelClass, scopes: true });
	...
	async create(ctx, next) {
		await this.authorize(ctx, { action: 'create', model: this.rest.modelClass });
