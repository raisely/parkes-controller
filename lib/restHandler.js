...
class RestHandler extends events.EventEmitter {
	...
	async create(ctx) {
		...
		const model = await this.modelClass.create(_.isObject(custom) ? custom : newRecord);
	...
	async update(ctx, record) {
		...
		const custom = await hook(this, 'beforeUpdate', ctx, newRecord); // bind hook

		// update record
		await record.update(_.isObject(custom) ? custom : newRecord);
