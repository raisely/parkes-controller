...
/**
  ...
  * @param {object} options.authorizationScope To push onto includes for scoping models to authorization (eg same user)
...
class RestHandler extends events.EventEmitter {
	...
	async find(ctx, _opts) {
		...
		if (this.authorizationWhere) {
			Object.assign(q.where, this.authorizationWhere);
		}
