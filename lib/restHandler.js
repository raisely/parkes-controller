...
class RestHandler extends events.EventEmitter {
	...
	async find(ctx, _opts) {
		...
		this.emit('beforeFind', ctx);

		const data = await this.modelClass.findOne(q);
		this.emit('afterFind', ctx, data);
	...
	async create(ctx) {
		...
		this.emit('beforeCreate', ctx, newRecord);
		await hook(this, 'beforeCreate', ctx, newRecord);
		const model = await this.modelClass.create(newRecord);
		this.emit('afterCreate', ctx, model);
	...
	async update(ctx, record) {
		...
		this.emit('beforeUpdate', ctx, record, newRecord);
		await hook(this, 'beforeUpdate', ctx, record, newRecord);
		// update record
		await record.update(newRecord);
		this.emit('afterUpdate', ctx, record);
	...
	async destroy(ctx, record) {
		this.emit('beforeDestroy', ctx, record);
		const data = await record.destroy();
		this.emit('afterDestroy', ctx, data);
