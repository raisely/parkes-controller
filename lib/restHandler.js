...
const { hook, hookHasThrown } = require('./hook');
...
class RestHandler extends events.EventEmitter {
	...
	async create(ctx) {
		...
		_hook = await hook(this, 'beforeCreate', ctx, newRecord); // bind hook
	...
	async update(ctx, record) {
		...
		_hook = await hook(this, 'beforeUpdate', ctx, newRecord); // bind hook
		if (hookHasThrown(_hook)) throw _hook;
